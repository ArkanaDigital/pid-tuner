...
fn with_client<T>(
    ...
    f: impl FnOnce(&mut dyn FlightController) -> std::result::Result<T, FcError>,
) -> R<T> {
    let mut g = state.client.lock().unwrap();
    let c = g
        .as_mut()
        .ok_or_else(|| "flight controller not connected".to_string())?;
    f(c.as_mut()).map_err(|e| e.to_string())
}

/// Take the client out of the state for a blocking operation, then put it back.
async fn with_client_blocking<T: Send + 'static>(
    state: &State<'_, AppState>,
    f: impl FnOnce(&mut dyn FlightController) -> std::result::Result<T, FcError> + Send + 'static,
) -> R<T> {
...
fn open_client(port: &str, kind: ConnectKind) -> std::result::Result<Client, FcError> {
    let try_mav = || -> std::result::Result<Client, FcError> {
        Ok(Box::new(fc_mavlink::MavClient::open(
            port,
            Duration::from_secs(3),
        )?))
    };
    let try_msp =
        || -> std::result::Result<Client, FcError> { Ok(Box::new(MspClient::open(port)?)) };
    match kind {
        ...
        ConnectKind::Auto => {
            ...
            match try_msp() {
                Ok(c) => Ok(c),
                Err(msp_err) => try_mav().map_err(|mav_err| {
                    FcError::Other(format!("MSP: {msp_err}; MAVLink: {mav_err}"))
                }),
            }
        }
    }
...
#[tauri::command]
pub async fn fc_connect(
    state: State<'_, AppState>,
    port: String,
    kind: Option<ConnectKind>,
) -> R<FcStatus> {
    let p = port.clone();
    let kind = kind.unwrap_or_default();
    let (client, mut status) =
        tauri::async_runtime::spawn_blocking(move || -> R<(Client, FcStatus)> {
    ...
    let snapshot: Option<(String, Vec<u8>)> = {
        ...
        let snap = match c.kind() {
            ...
            FcKind::Mavlink => c
                .backup()
                .ok()
                .map(|b| (format!("{}.{}", b.label, b.ext), b.bytes)),
        };
        *state.client.lock().unwrap() = Some(c);
        snap
    };
    if let Some(e) = state.engine.lock().unwrap().as_mut() {
        e.set_fc_tune(status.tune.clone(), status.firmware.clone())
            .map_err(|e| e.to_string())?;
...
pub fn fc_refresh(state: State<'_, AppState>) -> R<FcStatus> {
    ...
    if let Some(e) = state.engine.lock().unwrap().as_mut() {
        e.set_fc_tune(st.tune.clone(), st.firmware.clone())
            .map_err(|e| e.to_string())?;
    }
    *state.fc.lock().unwrap() = st.clone();
    Ok(st)
...
pub async fn fc_backup_cli(state: State<'_, AppState>) -> R<FcStatus> {
    ...
    if let Some(e) = state.engine.lock().unwrap().as_mut() {
        e.add_snapshot(&format!("{}.{}", b.label, b.ext), &b.bytes)
            .map_err(|e| e.to_string())?;
    }
...
pub async fn fc_preflight_fix(state: State<'_, AppState>) -> R<FcStatus> {
    ...
    if !r.verified {
        let bad: Vec<String> = r
            ...
            .iter()
            .filter(|o| !o.ok)
            .map(|o| format!("{} → {} (read back {:?})", o.param, o.wanted, o.read_back))
            .collect();
...
#[tauri::command]
pub async fn fc_download_import(
    app: AppHandle,
    state: State<'_, AppState>,
    which: Flight,
    log_id: Option<u32>,
    ...
    store
        .write_rel(session_id, &rel, &bytes)
        .map_err(|e| e.to_string())?;
...
pub async fn fc_apply(state: State<'_, AppState>, phase: ApplyPhase) -> R<FcApplyResult> {
    let recs: Vec<Recommendation> = state
        .engine
        .lock()
        .unwrap()
        .as_ref()
    ...
    let snapshot = {
        ...
        e.record_apply(phase, result.verified, method, notes)
            .map_err(|e| e.to_string())?;
        e.snapshot(Some(&fc))
    };
...
pub fn fc_export_text(state: State<'_, AppState>, phase: ApplyPhase) -> R<String> {
    ...
    let fw = state
        ...
        .firmware
        .clone()
        .or_else(|| e.session.firmware.clone());
    Ok(domain::fc::export_text_for(
        fw.as_ref(),
        e.session.recs(phase),
    ))
}
