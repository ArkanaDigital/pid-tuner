...
pub fn render(s: &Session, bundles: &[(Flight, AnalysisBundle)], images: &[ReportImage]) -> String {
    ...
    h.push_str("<!doctype html><html><head><meta charset='utf-8'><title>PID Tuning Report</title><style>");
    ...
    h.push_str(&format!(
        ...
        s.firmware.as_ref().map(|f| format!("{f:?}")).unwrap_or_default()
    ));
    ...
    for (title, recs) in [("Filter changes", &s.recs_filters), ("PID changes", &s.recs_pids)] {
    ...
    if !s.overrides.is_empty() {
        ...
        for o in &s.overrides {
            h.push_str(&format!("<li><b>{:?} / {}</b>: {}</li>", o.step, esc(&o.guard_id), esc(&o.reason)));
        }
        h.push_str("</ul>");
    }
    ...
    if !images.is_empty() {
        ...
        for im in images {
            h.push_str(&format!("<h3>{}</h3><img src='{}' alt='{}'>", esc(&im.title), im.data_url, esc(&im.title)));
        }
    }
    if !s.notes.trim().is_empty() {
        h.push_str(&format!("<h2>Notes</h2><p>{}</p>", esc(&s.notes).replace('\n', "<br>")));
    }
