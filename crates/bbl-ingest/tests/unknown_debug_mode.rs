...
fn unknown_debug_mode_id_still_ingests() {
    let Some(bytes) = fixture("bf_2025.12.2_speedybeef7v3_steadyhover.BFL") else {
        return;
    };
    ...
    let pos = bytes
        .windows(needle.len())
        .position(|w| w == needle)
        .expect("fixture has debug_mode 16");
    ...
    assert_eq!(
        b.meta.headers.get("bf.debug_mode_raw").map(String::as_str),
        Some("97")
    );
...
fn prerelease_firmware_suffix_is_accepted() {
    ...
    assert!(
        matches!(&log.firmware, domain::Firmware::Betaflight { version, .. } if version.starts_with("2026.6.0"))
    );
}
