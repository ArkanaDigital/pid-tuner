...
fn list<T: std::str::FromStr + Copy>(h: &H, key: &str, n: usize) -> Option<Vec<T>> {
    let v: Vec<T> = h.get(key)?.split(',').filter_map(|s| s.trim().parse().ok()).collect();
    (v.len() >= n).then_some(v)
}

fn ftype(h: &H, key: &str, default: BfFilterType) -> BfFilterType {
    h.get(key).and_then(|v| BfFilterType::from_cli(v)).unwrap_or(default)
}
...
pub fn parse_bf_tune(h: &H) -> BfTune {
    ...
    t.anti_gravity_gain = num::<u32>(h, "anti_gravity_gain").map(|v| v.min(u16::MAX as u32) as u16).unwrap_or(t.anti_gravity_gain);
    ...
    t.feedforward_smooth_factor = num(h, "feedforward_smooth_factor").unwrap_or(t.feedforward_smooth_factor);
    t.feedforward_jitter_factor = num(h, "feedforward_jitter_factor").unwrap_or(t.feedforward_jitter_factor);
    ...
    let f = &mut t.filters;
    f.gyro_lpf1_static_hz = u16_or(h, "gyro_lpf1_static_hz", "gyro_lowpass_hz", f.gyro_lpf1_static_hz);
    ...
    f.gyro_lpf2_static_hz = u16_or(h, "gyro_lpf2_static_hz", "gyro_lowpass2_hz", f.gyro_lpf2_static_hz);
    ...
    f.dterm_lpf1_static_hz = u16_or(h, "dterm_lpf1_static_hz", "dterm_lowpass_hz", f.dterm_lpf1_static_hz);
    ...
    f.dterm_lpf2_static_hz = u16_or(h, "dterm_lpf2_static_hz", "dterm_lowpass2_hz", f.dterm_lpf2_static_hz);
    ...
    f.rpm_filter_harmonics = num(h, "rpm_filter_harmonics").or_else(|| num(h, "gyro_rpm_notch_harmonics")).unwrap_or(f.rpm_filter_harmonics);
    f.rpm_filter_min_hz = num(h, "rpm_filter_min_hz").or_else(|| num(h, "gyro_rpm_notch_min")).unwrap_or(f.rpm_filter_min_hz);
    f.rpm_filter_q = num(h, "rpm_filter_q").or_else(|| num(h, "gyro_rpm_notch_q")).unwrap_or(f.rpm_filter_q);
    f.rpm_filter_fade_range_hz = num(h, "rpm_filter_fade_range_hz").unwrap_or(f.rpm_filter_fade_range_hz);
    ...
    s.gyro_filter = num::<u8>(h, "simplified_gyro_filter").map(|v| v != 0).unwrap_or(s.gyro_filter);
    s.gyro_filter_multiplier = num(h, "simplified_gyro_filter_multiplier").unwrap_or(s.gyro_filter_multiplier);
    s.dterm_filter = num::<u8>(h, "simplified_dterm_filter").map(|v| v != 0).unwrap_or(s.dterm_filter);
    s.dterm_filter_multiplier = num(h, "simplified_dterm_filter_multiplier").unwrap_or(s.dterm_filter_multiplier);
    t
}
