...
fn chirp_log_sets_pid_source_and_guards_report_incoherent_axes() {
    ...
    let mut e = SessionEngine::create(SessionStore::new(d.path()), "chirp".into(), Mode::Offline).unwrap();
    ...
    let cq = g.iter().find(|x| x.id == "chirp_quality").expect("chirp_quality guard");
    match &cq.outcome {
        GuardOutcome::Fail { message, fix_hint } => {
            assert!(message.contains("roll") && message.contains("pitch") && !message.contains("yaw"), "{message}");
    ...
    assert!(names.iter().all(|n| !n.contains("filter") && !n.contains("lpf") && !n.contains("notch")), "{names:?}");
...
fn chirp_quality_guard_absent_without_chirp_data() {
    let d = tempfile::tempdir().unwrap();
    let e = SessionEngine::create(SessionStore::new(d.path()), "plain".into(), Mode::Offline).unwrap();
