...
fn guard<'a>(g: &'a [GuardResult], id: &str) -> &'a GuardResult {
    g.iter().find(|x| x.id == id).unwrap_or_else(|| {
        panic!(
            "guard {id} missing in {:?}",
            g.iter().map(|x| &x.id).collect::<Vec<_>>()
        )
    })
}
...
fn ten_hz_pid_log_fails_loop_rate_guard_with_bit0_hint() {
    let Some(bytes) = fixture("copter_4.6.3_quad_pid_msgs.bin") else {
        return;
    };
    let (_d, mut e) = engine();
    ...
    assert!(
        g.iter().all(|x| x.id != "log_rate" && x.id != "steps"),
        "{:?}",
        g.iter().map(|x| &x.id).collect::<Vec<_>>()
    );
    ...
    match &pr.outcome {
        GuardOutcome::Fail { message, fix_hint } => {
            assert!(
                message.contains("10 Hz") && message.contains("400 Hz"),
                "{message}"
            );
            assert!(
                fix_hint.as_deref().unwrap().contains("bit 0"),
                "{fix_hint:?}"
            );
        }
        o => panic!("{o:?}"),
    ...
    assert!(
        guard(&g, "ap_saturation").satisfied(),
        "{:?}",
        guard(&g, "ap_saturation").outcome
    );
}

#[test]
fn batch_sampler_log_passes_isbh_and_hover_guards() {
    let Some(bytes) = fixture("copter_4.5.5_tarot_x4_batch_imu.bin") else {
        return;
    };
    let (_d, mut e) = engine();
    ...
    assert!(
        guard(&g, "ap_isbh").satisfied(),
        "{:?}",
        guard(&g, "ap_isbh").outcome
    );
    assert!(
        guard(&g, "ap_hover").satisfied(),
        "{:?}",
        guard(&g, "ap_hover").outcome
    );
...
fn no_batch_log_fails_isbh_guard() {
    let Some(bytes) = fixture("copter_4.4.2_auto_jitter.bin") else {
        return;
    };
    let (_d, mut e) = engine();
    ...
    match &guard(&g, "ap_isbh").outcome {
        GuardOutcome::Fail { fix_hint, .. } => {
            assert!(fix_hint.as_deref().unwrap().contains("INS_LOG_BAT_MASK"))
        }
        o => panic!("{o:?}"),
    }
}
...
fn autotune_result_guard_detects_unsaved_axes() {
    ...
    match &guard(&g, "autotune_result").outcome {
        GuardOutcome::Fail { message, .. } => {
            assert!(
                message.contains("RLL") && message.contains("PIT") && message.contains("unchanged"),
                "{message}"
            );
        }
        o => panic!("{o:?}"),
...
fn fc_supported_and_preflight_bits_follow_ardupilot_defines() {
    ...
    fc.firmware = Some(Firmware::ArduCopter {
        version: "4.3.8".into(),
    });
    let supported =
        |fc: &FcStatus| guard(&e.guards(Step::Connect, Some(fc)), "fc_supported").satisfied();
    assert!(!supported(&fc));
    fc.firmware = Some(Firmware::ArduCopter {
        version: "4.5.5".into(),
    });
    ...
    assert!(
        g.iter()
            .all(|x| x.id != "log_rate" && x.id != "raw_gyro_logging"),
        "{:?}",
        g.iter().map(|x| &x.id).collect::<Vec<_>>()
    );
    ...
    fc.log_bitmask = Some(
        180222
            | domain::ap_consts::LOG_BIT_ATTITUDE_FAST
            | domain::ap_consts::LOG_BIT_PID
            | domain::ap_consts::LOG_BIT_IMU_RAW,
    );
