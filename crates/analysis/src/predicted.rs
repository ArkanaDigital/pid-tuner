...
pub fn chain_from_params(
    ...
    for prefix in ["INS_HNTCH_", "INS_HNTC2_"] {
        ...
        let composite = if opts & 0b1 != 0 {
            2
        } else if opts & 0b10000 != 0 {
            3
        } else {
            1
        };
...
pub fn predict(pre: &Spectrum, chain: &FilterChain) -> Spectrum {
    Spectrum {
        axis: pre.axis,
        kind: SpectrumKind::Predicted,
        f_hz: pre.f_hz.clone(),
        psd_db: chain.predict_psd_db(&pre.f_hz, &pre.psd_db),
        nfft: pre.nfft,
        fs_hz: pre.fs_hz,
    }
}
...
mod tests {
    ...
    fn notch_prediction_attenuates_peak() {
        ...
        assert!(
            p2.psd_db[113] < -25.0 && p2.psd_db[80] > -16.0,
            "{} {}",
            p2.psd_db[113],
            p2.psd_db[80]
        );
