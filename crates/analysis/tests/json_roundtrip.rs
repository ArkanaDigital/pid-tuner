...
fn step_response_nan_latency_roundtrips_as_null() {
    let s = domain::StepResponse {
        axis: domain::Axis::Roll,
        variant: domain::StepVariant::PtStep,
        t_ms: vec![],
        mean: vec![],
        p10: vec![],
        p90: vec![],
    ...
    let s2 = domain::StepResponse {
        latency_ms: 19.5,
        ..s
    };
    let back: domain::StepResponse =
...
fn bundle_json_roundtrip_on_hover_log() {
    ...
    let Some(bytes) = fixture("bf/bf_2025.12.2_speedybeef7v3_steadyhover.BFL")
        .or_else(|| fixture("ap/copter_4.6.3_quad_pid_msgs.bin"))
    else {
        return;
    };
    ...
    let expected_nulls = b.steps.iter().filter(|s| s.latency_ms.is_nan()).count()
        + b.steps.iter().filter(|s| s.settle_ms.is_none()).count()
        + usize::from(b.quality.airborne_range_s.is_none())
        + usize::from(b.quality.pid_rate_hz.is_none())
        + usize::from(b.quality.max_pid_out.is_none())
        + b.anomalies
            .iter()
            .map(|a| usize::from(a.axis.is_none()) + usize::from(a.motor.is_none()))
            .sum::<usize>()
        + b.freq_resp
            .iter()
            .map(|fr| {
                ...
                let t: usize = m
                    ...
                    .iter()
                    .map(|t| {
                        [t.crossover_hz, t.gain_to_target, t.gain_for_sens_limit]
                            .iter()
    ...
    assert!(
        ...
        "unexpected nulls in bundle JSON: {nulls} > {expected_nulls}"
    );
}
...
fn frequency_response_nan_vectors_roundtrip() {
    let fr = domain::FrequencyResponse {
        ...
        metrics: domain::FrMetrics {
            ...
            phase_margin_deg: f32::NAN,
            max_phase_margin_deg: 80.0,
            resonant_peak_db: 1.0,
            resonant_peak_hz: 30.0,
            loop_delay_ms: f32::NAN,
            low_freq_err_db: 0.1,
            coherence_mean: 0.9,
            noise_floor_hz: f32::NAN,
            sens_peak_db: 2.0,
            sens_peak_hz: 40.0,
            step_overshoot: 1.05,
            step_rise_ms: 12.0,
            step_settle_ms: 40.0,
            targets: vec![domain::FrTarget {
                pm_deg: 60.0,
                crossover_hz: f32::NAN,
                gain_to_target: f32::NAN,
                gain_for_sens_limit: 1.2,
            }],
        },
    };
