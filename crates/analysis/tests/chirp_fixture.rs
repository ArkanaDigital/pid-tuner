...
fn chirp_fixture_segments_and_yaw_metrics() {
    ...
    assert!(
        (cfg.f_start_hz - 0.2).abs() < 1e-6
            && (cfg.f_end_hz - 600.0).abs() < 1e-6
            && cfg.time_s == 20.0
            && cfg.amplitude == [230, 230, 180]
    );
    ...
    for s in &chirp.segments {
        ...
        assert!(
            (s.f_start_hz - 0.2).abs() < 0.05 && s.f_end_hz > 590.0,
            "{s:?}"
        );
    ...
    assert!(
        ((m.bandwidth_hz - 20.2) / 20.2).abs() < 0.02,
        "bandwidth {}",
        m.bandwidth_hz
    );
    assert!(
        ((m.crossover_hz - 12.6) / 12.6).abs() < 0.02,
        "crossover {}",
        m.crossover_hz
    );
    assert!(
        (m.phase_margin_deg - 66.1).abs() < 1.0,
        "PM {}",
        m.phase_margin_deg
    );
    assert!(
        (m.resonant_peak_db - 0.7).abs() < 0.2,
        "Mr {}",
        m.resonant_peak_db
    );
    assert!(
        (m.loop_delay_ms - 4.67).abs() < 0.3,
        "delay {}",
        m.loop_delay_ms
    );
    ...
    assert!(
        b.quality.chirp_coherence_per_axis[2] > 0.9 && b.quality.chirp_coherence_per_axis[0] < 0.6
    );
    // the excitation lives in the setpoint: no phantom oscillation anomalies
    assert!(
        b.anomalies
            .iter()
            .all(|a| a.kind != AnomalyKind::Oscillation),
        "{:?}",
        b.anomalies
    );
}
