...
impl MavClient {
    ...
    pub fn open(address: &str, timeout: Duration) -> Result<Self, FcError> {
        ...
        c.reopen = Some(Box::new(crate::link::open));
        Ok(c)
    }
    ...
    pub fn pump(&mut self, timeout: Duration) -> Result<Option<MavMessage>, FcError> {
        ...
        let Some((hdr, msg)): Option<Frame> = self.link.recv(timeout)? else {
            return Ok(None);
        };
        ...
        match &msg {
            MavMessage::HEARTBEAT(h) => {
                if h.autopilot != MavAutopilot::MAV_AUTOPILOT_INVALID
                    && h.mavtype != MavType::MAV_TYPE_GCS
                {
            ...
            MavMessage::STATUSTEXT(t) => {
                ...
                if s.starts_with("ArduCopter V")
                    || s.starts_with("ArduPlane V")
                    || s.starts_with("ArduRover V")
                {
    ...
    pub fn wait_for<T>(
        &mut self,
        timeout: Duration,
        mut pred: impl FnMut(&MavMessage) -> Option<T>,
    ) -> Result<Option<T>, FcError> {
    ...
    fn wait_heartbeat(&mut self, timeout: Duration) -> Result<(), FcError> {
        ...
        self.wait_for(timeout, |_| None::<()>).map(|_| ())?;
        match self.last_heartbeat.as_ref().map(|h| h.0) {
            Some(t) if Some(t) != before => Ok(()),
            _ => Err(FcError::Timeout(format!(
                "no MAVLink heartbeat from an autopilot on {} within {:?}",
    ...
    fn request_version(&mut self) {
        ...
        let _ = self.wait_for(Duration::from_millis(1500), |m| {
            matches!(m, MavMessage::AUTOPILOT_VERSION(_)).then_some(())
        });
    }
    ...
    pub fn firmware(&self) -> Firmware {
        ...
        let is_apm = self
            .last_heartbeat
            .as_ref()
            .map(|h| h.1.autopilot == MavAutopilot::MAV_AUTOPILOT_ARDUPILOTMEGA)
            .unwrap_or(false);
        match (is_apm, mavtype) {
            (true, Some(t)) if is_copter(t) => Firmware::ArduCopter { version: ver },
            (true, Some(t)) => Firmware::Unknown {
                product: format!("ArduPilot {t:?} {ver}"),
            },
            _ => Firmware::Unknown {
                product: format!("MAVLink {ver}"),
            },
        }
    }
    ...
    pub fn fetch_all_params(
        &mut self,
        mut progress: impl FnMut(usize, usize),
    ) -> Result<&ParamStore, FcError> {
        let (sys, comp) = self.target;
        ...
        if self.store.count.is_none() {
            return Err(FcError::Timeout(
                "no PARAM_VALUE received for PARAM_REQUEST_LIST".into(),
            ));
        }
        ...
        if !self.store.is_complete() {
            return Err(FcError::Timeout(format!(
    ...
    pub fn set_param(&mut self, name: &str, value: f32) -> Result<ApplyOutcome, FcError> {
        ...
        let wanted = if crate::params::is_integer_type(ptype) {
            value.round()
        } else {
            value
        };
        ...
        let ok = read_back
            .map(|v| values_match(ptype, wanted, v))
            .unwrap_or(false);
        Ok(ApplyOutcome {
            param: name.into(),
            wanted: crate::params::fmt_value(ptype, wanted),
            read_back: read_back.map(|v| crate::params::fmt_value(ptype, v)),
            ok,
            via: "mavlink".into(),
        })
    }
    ...
    fn status_from_store(&self) -> FcStatus {
        ...
        let pid_fast = bitmask.map(|m| {
            m & (LOG_BIT_ATTITUDE_FAST | LOG_BIT_PID) == (LOG_BIT_ATTITUDE_FAST | LOG_BIT_PID)
        });
...
impl FlightController for MavClient {
    ...
    fn poll(&mut self) -> Result<FcStatus, FcError> {
        ...
        if self.heartbeat_age().map(|a| a > 1.5).unwrap_or(true) {
            let _ = self.wait_for(Duration::from_millis(1200), |m| {
                matches!(m, MavMessage::HEARTBEAT(_)).then_some(())
            });
        }
    ...
    fn backup(&mut self) -> Result<Backup, FcError> {
        self.fetch_all_params(|_, _| {})?;
        let hdr = format!(
            "PID Tuner parameter backup — {:?} — {} params",
            self.firmware(),
            self.store.params.len()
        );
        Ok(Backup {
            label: "params".into(),
            ext: "param".into(),
            bytes: self.store.to_param_text(&hdr).into_bytes(),
        })
    }
    ...
    fn preflight_fix(&mut self, fix: PreflightFix) -> Result<ApplyResult, FcError> {
        ...
        let mut plan: Vec<(&str, f32)> = vec![
            ("LOG_BITMASK", want_mask as f32),
            ("INS_LOG_BAT_MASK", 1.0),
            ("INS_LOG_BAT_OPT", (INS_LOG_BAT_OPT_PRE_POST) as f32),
        ...
        plan.retain(|(n, v)| {
            self.store
                .value(n)
                .map(|c| !values_match(self.store.get(n).unwrap().ptype, *v, c))
                .unwrap_or(true)
        });
    ...
    fn apply(&mut self, recs: &[Recommendation]) -> Result<ApplyResult, FcError> {
        ...
        let (gate, rest): (Vec<&Recommendation>, Vec<&Recommendation>) = accepted
            .iter()
            .partition(|r| r.param.name().ends_with("_ENABLE"));
        let mut outcomes = Vec::new();
        let mut rebooted = false;
        for r in &gate {
    ...
    fn reboot_and_reconnect(&mut self) -> Result<(), FcError> {
        ...
        self.reopen = Some(reopen);
        Err(FcError::Timeout(format!(
            "flight controller did not come back after reboot ({last_err:?})"
        )))
    }
...
mod tests {
    ...
    fn firmware_version_decodes_like_send_autopilot_version() {
        ...
        assert_eq!(fw_version_string((4 << 24 | 6 << 16) | 192), "4.6.0-rc");
    }
}
