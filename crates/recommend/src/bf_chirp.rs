...
pub fn scales_for(fr: &FrequencyResponse) -> Result<AxisScales, String> {
    ...
    if fr.n_windows < MIN_WINDOWS {
        return Err(format!("{}: only {} Welch windows (need ≥ {MIN_WINDOWS}); fly longer or more sweeps.", fr.axis.name(), fr.n_windows));
    }
    if !(m.coherence_mean >= MIN_COHERENCE) {
    ...
    let t60 = m.targets.iter().find(|t| (t.pm_deg - TARGET_PM_DEG).abs() < 0.1);
    let gain_to_target = t60.map(|t| t.gain_to_target).unwrap_or(f32::NAN);
    if !(m.crossover_hz.is_finite() && m.phase_margin_deg.is_finite() && m.resonant_peak_db.is_finite()) {
        return Err(format!("{}: no usable crossover/phase margin (open loop not coherent between 2 Hz and the crossover).", fr.axis.name()));
    }
    let gain_for_margin = if gain_to_target.is_finite() { gain_to_target } else { 1.0 };
    ...
    let robustness = if requested.clamp(SCALE_LO, SCALE_HI) > 0.0 { pi / requested.clamp(SCALE_LO, SCALE_HI) } else { 1.0 };
    let i = if m.low_freq_err_db.is_finite() { (10f32.powf(-m.low_freq_err_db / 20.0)).clamp(0.85, 1.2) } else { 1.0 };
    let ff = (gain_for_margin * ff_backoff * robustness).clamp(0.8, 1.2);
    let _ = MAX_SENSITIVITY_PEAK;
    Ok(AxisScales { axis: fr.axis, pi, i, ff, coherence: m.coherence_mean, bandwidth_hz: m.bandwidth_hz, phase_margin_deg: m.phase_margin_deg, sens_backoff })
}
...
fn conf(s: &AxisScales, scale: f32) -> Confidence {
    if s.coherence >= 0.8 && (scale - 1.0).abs() <= 0.15 { Confidence::High } else { Confidence::Medium }
}
...
pub fn rules(t: &BfTune, b: &AnalysisBundle) -> (Vec<Recommendation>, [bool; 3]) {
    ...
    for fr in &b.freq_resp {
        match scales_for(fr) {
            ...
            Err(why) => out.push(Recommendation {
                ...
                evidence: vec![EvidenceRef::FreqResp { axis: fr.axis, bandwidth_hz: fr.metrics.bandwidth_hz, phase_margin_deg: fr.metrics.phase_margin_deg, coherence: fr.metrics.coherence_mean }],
    ...
    let ev = |s: &AxisScales| vec![EvidenceRef::FreqResp { axis: s.axis, bandwidth_hz: s.bandwidth_hz, phase_margin_deg: s.phase_margin_deg, coherence: s.coherence }];
    ...
    if t.simplified.pids_mode == 0 {
        for s in &scales {
            ...
            let pid = t.pids[k];
            if changed(s.pi) {
                out.push(rec(&format!("p_{ax}"), ParamValue::U8(pid.p), ParamValue::U8(clamp_u8((pid.p as f32 * s.pi).round() as i32, 1, 250)), why(s, "P", s.pi), ev(s), conf(s, s.pi)));
            }
            let i_scale = s.pi * s.i;
            if changed(i_scale) {
                out.push(rec(&format!("i_{ax}"), ParamValue::U8(pid.i), ParamValue::U8(clamp_u8((pid.i as f32 * i_scale).round() as i32, 1, 250)), why(s, "I (with P, plus low-frequency error)", i_scale), ev(s), conf(s, i_scale)));
            }
            if pid.ff > 0 && changed(s.ff) {
                out.push(rec(&format!("f_{ax}"), ParamValue::U16(pid.ff), ParamValue::U16(clamp_u16((pid.ff as f32 * s.ff).round() as i32, 0, 1000)), why(s, "FF", s.ff), ev(s), conf(s, s.ff)));
            }
        }
    } else {
        // Simplified sliders ON: scale the sliders instead of the raw values, never touch D or filters.
        let sm = &t.simplified;
        let base = scales.iter().find(|s| s.axis == Axis::Roll).or_else(|| scales.iter().find(|s| s.axis == Axis::Pitch));
        if let Some(b0) = base {
            let sl = |cur: u8, scale: f32| ParamValue::U8(clamp_u8((cur as f32 * scale).round() as i32, 25, 250));
            if changed(b0.pi) {
                out.push(rec("simplified_pi_gain", ParamValue::U8(sm.pi_gain), sl(sm.pi_gain, b0.pi), why(b0, "PI slider", b0.pi), ev(b0), conf(b0, b0.pi)));
            }
            if changed(b0.i) {
                out.push(rec("simplified_i_gain", ParamValue::U8(sm.i_gain), sl(sm.i_gain, b0.i), why(b0, "I slider", b0.i), ev(b0), conf(b0, b0.i)));
            }
            if changed(b0.ff) {
                out.push(rec("simplified_feedforward_gain", ParamValue::U8(sm.feedforward_gain), sl(sm.feedforward_gain, b0.ff), why(b0, "FF slider", b0.ff), ev(b0), conf(b0, b0.ff)));
            }
            if let (Some(p), true) = (scales.iter().find(|s| s.axis == Axis::Pitch), b0.axis == Axis::Roll) {
                let ratio = p.pi / b0.pi;
                if changed(ratio) {
                    out.push(rec("simplified_pitch_pi_gain", ParamValue::U8(sm.pitch_pi_gain), sl(sm.pitch_pi_gain, ratio), why(p, "pitch PI slider relative to roll", ratio), ev(p), conf(p, ratio)));
                }
            }
        }
    }
...
mod tests {
    ...
    fn fr(axis: Axis, coh: f32, windows: usize, pm: f32, gain60: f32, sens_gain: f32, mr: f32, lf: f32) -> FrequencyResponse {
        FrequencyResponse {
            axis,
            angle_mode: false,
            f_hz: vec![],
            ...
            metrics: FrMetrics {
                ...
                step_settle_ms: 40.0,
                targets: vec![
                    FrTarget { pm_deg: 50.0, crossover_hz: 45.0, gain_to_target: gain60 * 1.2, gain_for_sens_limit: sens_gain },
                    FrTarget { pm_deg: 60.0, crossover_hz: 35.0, gain_to_target: gain60, gain_for_sens_limit: sens_gain },
                    FrTarget { pm_deg: 72.5, crossover_hz: 20.0, gain_to_target: gain60 * 0.7, gain_for_sens_limit: sens_gain },
                ],
            },
        }
    }
    ...
    fn tune() -> BfTune {
        ...
        t.pids = [BfAxisPid { p: 45, i: 80, d: 30, ff: 120, d_max: 40 }, BfAxisPid { p: 47, i: 84, d: 34, ff: 125, d_max: 46 }, BfAxisPid { p: 45, i: 80, d: 0, ff: 120, d_max: 0 }];
        t
    }

    fn bundle(frs: Vec<FrequencyResponse>) -> AnalysisBundle {
        AnalysisBundle { log: LogId("x".into()), quality: LogQuality::default(), steps: vec![], spectra: vec![], spectrograms: vec![], peaks: vec![], anomalies: vec![], freq_resp: frs }
    }

    #[test]
    fn gate_blocks_low_coherence_few_windows_and_angle_mode() {
        let t = tune();
        let (r, cov) = rules(&t, &bundle(vec![fr(Axis::Roll, 0.5, 50, 55.0, 1.3, 2.0, 1.0, 0.0)]));
        assert!(!cov[0] && r.len() == 1 && r[0].reason.contains("coherence"));
        let (r, cov) = rules(&t, &bundle(vec![fr(Axis::Roll, 0.9, 5, 55.0, 1.3, 2.0, 1.0, 0.0)]));
    ...
    fn gain_to_target_is_clamped_and_backed_off_by_resonance_and_sensitivity() {
        ...
        let (r, cov) = rules(&t, &bundle(vec![fr(Axis::Roll, 0.9, 50, 40.0, 1.8, 2.0, 1.0, 0.0)]));
        assert!(cov[0]);
    ...
    fn small_changes_are_skipped_and_d_is_never_touched() {
        let t = tune();
        let (r, cov) = rules(&t, &bundle(vec![fr(Axis::Roll, 0.95, 50, 60.0, 1.01, 2.0, 0.5, 0.0)]));
        assert!(cov[0]);
        assert!(r.is_empty(), "{r:?}");
        let (r, _) = rules(&t, &bundle(vec![fr(Axis::Pitch, 0.95, 50, 45.0, 1.15, 2.0, 0.5, -2.0)]));
        let names: Vec<&str> = r.iter().map(|x| x.param.name()).collect();
        assert!(names.contains(&"p_pitch") && names.contains(&"i_pitch") && names.contains(&"f_pitch"), "{names:?}");
        ...
        assert_eq!(i.new, ParamValue::U8((84.0f32 * 1.15 * 1.2).round() as u8)); // lf scale clamped to 1.2
    }
    ...
    fn slider_path_emits_only_slider_names_and_no_filters() {
        ...
        let (r, _) = rules(&t, &bundle(vec![fr(Axis::Roll, 0.9, 50, 45.0, 1.15, 2.0, 0.5, 0.0), fr(Axis::Pitch, 0.9, 50, 45.0, 1.25, 2.0, 0.5, 0.0)]));
        let names: Vec<&str> = r.iter().map(|x| x.param.name()).collect();
        assert!(names.contains(&"simplified_pi_gain") && names.contains(&"simplified_feedforward_gain") && names.contains(&"simplified_pitch_pi_gain"), "{names:?}");
        assert!(names.iter().all(|n| n.starts_with("simplified_")), "{names:?}");
        assert!(!names.iter().any(|n| n.contains("filter") || n.contains("lpf") || n.contains("notch") || n.contains("dterm") || *n == "simplified_d_gain" || *n == "simplified_d_max_gain" || n.contains("pids_mode")), "{names:?}");
        let pp = r.iter().find(|x| x.param.name() == "simplified_pitch_pi_gain").unwrap();
        assert_eq!(pp.new, ParamValue::U8((100.0f32 * 1.25 / 1.15).round() as u8));
    }
}
