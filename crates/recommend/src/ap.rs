...
fn meta_or_default(name: &str) -> ParamMeta {
    lookup(name).copied().unwrap_or(ParamMeta {
        name: "",
        min: None,
        max: None,
        increment: None,
...
/// Clamp to the pdef range / increment of `name` (unknown names pass through).
pub fn clamp_ap(name: &str, v: f32) -> f32 {
    clamp(name, v)
}

/// Bounds of an ArduPilot parameter from the pdef table.
pub fn ap_bounds(name: &str) -> Option<&'static ParamMeta> {
    lookup(name)
}
...
fn clamp(name: &str, v: f32) -> f32 {
    ...
    if let Some(inc) = m.increment {
        if inc > 0.0 {
            x = (x / inc).round() * inc;
            // Re-round to the increment's decimals so f32 noise (30 * 0.001 =
            // 0.030000001) does not leak into the value we display and write.
            let dec = (-(inc as f64).log10()).ceil().max(0.0) as i32;
            let scale = 10f64.powi(dec);
            x = ((x as f64 * scale).round() / scale) as f32;
        }
    }
    if m.integer {
        x = x.round();
...
fn requires_reboot(name: &str) -> bool {
    ...
    name.ends_with("_ENABLE") && name.starts_with("INS_HNTC")
        || meta_or_default(name).reboot_required
}
...
pub fn filters(t: &ApTune, b: &AnalysisBundle, hover_thr: Option<f32>) -> Vec<Recommendation> {
    ...
    let raw_peaks: Vec<&NoisePeak> = b
        .peaks
        .iter()
        .filter(|p| p.kind == SpectrumKind::GyroRaw && p.f_hz >= 40.0 && p.f_hz <= 400.0)
        .collect();
    if let Some(peak) = raw_peaks
        .iter()
        .max_by(|a, c| a.prominence_db.partial_cmp(&c.prominence_db).unwrap())
    {
        ...
        let filt_peak = b.peaks.iter().find(|p| {
            p.kind == SpectrumKind::GyroFilt
                && p.axis == peak.axis
                && (p.f_hz - peak.f_hz).abs() < 10.0
        });
        let survives = filt_peak
            .map(|p| p.prominence_db >= MIN_NOTCH_GAIN_DB)
            .unwrap_or(!enabled);
        if survives {
            let ev = vec![EvidenceRef::Peak {
                axis: peak.axis,
                f_hz: peak.f_hz,
                psd_db: peak.psd_db,
            }];
            ...
            let mode =
                if b.quality.has_pid_terms && g("INS_HNTCH_MODE").map(|m| m as i32) == Some(3) {
                    3.0
                } else {
                    1.0
                };
            ...
            out.push(rec(
                "INS_HNTCH_REF",
                g("INS_HNTCH_REF").unwrap_or(0.0),
                reference,
                format!("Reference = hover throttle ({reference:.3}, CTUN.ThH / MOT_THST_HOVER)."),
                ev.clone(),
                Confidence::Medium,
            ));
            out.push(rec(
                ...
                cur_freq,
                peak.f_hz,
                format!("Notch centre = hover motor frequency {:.0} Hz.", peak.f_hz),
                ev.clone(),
                Confidence::High,
            ));
            out.push(rec(
                "INS_HNTCH_BW",
                g("INS_HNTCH_BW").unwrap_or(0.0),
                peak.f_hz / 2.0,
                "Bandwidth = FREQ/2 (ArduPilot notch setup guidance).",
                ev.clone(),
                Confidence::Medium,
            ));
            out.push(rec(
                "INS_HNTCH_ATT",
                g("INS_HNTCH_ATT").unwrap_or(0.0),
                40.0,
                "40 dB attenuation (default recommendation).",
                ev.clone(),
                Confidence::Medium,
            ));
            out.push(rec(
                "INS_HNTCH_HMNCS",
                g("INS_HNTCH_HMNCS").unwrap_or(0.0),
                3.0,
                "1st + 2nd harmonic (bitmask 3) for a quad; reboot required.",
                ev,
                Confidence::Medium,
            ));
        }
...
pub fn pids(t: &ApTune, b: &AnalysisBundle, max_srate: Option<[f32; 3]>) -> Vec<Recommendation> {
    ...
    for s in &b.steps {
        ...
        let k = s.axis.index();
        let (pn, in_, dn) = (
            format!("ATC_RAT_{ax}_P"),
            format!("ATC_RAT_{ax}_I"),
            format!("ATC_RAT_{ax}_D"),
        );
        let (Some(p), Some(i)) = (g(&pn), g(&in_)) else {
            continue;
        };
        ...
        if let Some(sr) = max_srate.map(|m| m[k]) {
            if sr > osc_smax {
                ...
                out.push(rec(
                    ...
                    i,
                    i * f,
                    "I follows P (Quicktune keeps the P:I ratio).",
                    vec![],
                    Confidence::High,
                ));
                if d > 0.0 {
        ...
        if s.overshoot > OVERSHOOT_HI {
            if s.axis != Axis::Yaw && d > 0.0 {
                out.push(rec(
                    ...
                    d * (1.0 + D_STEP),
                    format!(
                        "{} overshoots to {:.2} (target ≤ 1.10); more D damps it.",
                        s.axis.name(),
                        s.overshoot
                    ),
                    ev.clone(),
            ...
            if s.overshoot > 1.35 || s.axis == Axis::Yaw {
                out.push(rec(
                    ...
                    p,
                    p * (1.0 - P_STEP),
                    format!("{} overshoot {:.2}: reduce P.", s.axis.name(), s.overshoot),
                    ev.clone(),
                    Confidence::Medium,
                ));
                out.push(rec(
                    ...
                    i,
                    i * (1.0 - P_STEP),
                    "I follows P (keep the P:I ratio).",
                    vec![],
                    Confidence::Medium,
                ));
            }
        } else if s.overshoot < OVERSHOOT_OK
            && s.latency_ms.is_finite()
            && s.latency_ms > SLOW_LATENCY_MS
            && !saturated
        {
            out.push(rec(
                &pn,
                ...
                p * (1.0 + P_STEP),
                format!(
                    "{} reaches 50 % only after {:.0} ms with no overshoot; raise P.",
                    s.axis.name(),
                    s.latency_ms
                ),
                ev.clone(),
        ...
        if s.steady_state < SS_LOW && !saturated {
            out.push(rec(
                ...
                i * (1.0 + I_STEP),
                format!(
                    "{} settles at {:.2}; raise I to remove the steady-state error.",
                    s.axis.name(),
                    s.steady_state
                ),
                ev.clone(),
        ...
        if saturated {
            out.push(rec(
                ...
                p * (1.0 - P_STEP),
                format!(
                    "{} mixer output saturates (RATE.*Out {:.2}); only gain reductions are safe.",
                    s.axis.name(),
                    sat[k]
                ),
                vec![EvidenceRef::Quality {
                    field: "max_pid_out".into(),
                    value: sat[k] as f64,
                }],
                Confidence::Medium,
            ));
...
fn dedupe(v: Vec<Recommendation>) -> Vec<Recommendation> {
    ...
    for r in v {
        if let Some(e) = out.iter_mut().find(|e| e.param == r.param) {
            let ratio = if e.old.as_f64().abs() > 1e-12 {
                r.new.as_f64() / e.old.as_f64()
            } else {
                1.0
            };
...
pub fn max_srate(log: &FlightLog, range_s: Option<(f32, f32)>) -> Option<[f32; 3]> {
    ...
    for (k, ax) in log.axes.iter().enumerate() {
        ...
        out[k] = s[i0.min(s.len())..i1.min(s.len())]
            .iter()
...
mod tests {
    ...
    fn hover_peak_enables_harmonic_notch_with_reboot_flags() {
        let b = bundle(
            vec![NoisePeak {
                ...
                psd_db: 5.0,
                prominence_db: 25.0,
                band: NoiseBand::Control,
            }],
            vec![],
        );
        let r = filters(&tune(), &b, Some(0.35));
        let by = |n: &str| {
            r.iter().find(|x| x.param.name() == n).unwrap_or_else(|| {
                panic!(
                    "{n} missing: {:?}",
                    r.iter().map(|x| x.param.name()).collect::<Vec<_>>()
                )
            })
        };
        ...
        for x in &r {
            let m = lookup(x.param.name())
                .unwrap_or_else(|| panic!("{} not in pdef table", x.param.name()));
            if let Some(lo) = m.min {
                assert!(x.new.as_f64() >= lo as f64);
            }
            if let Some(hi) = m.max {
                assert!(x.new.as_f64() <= hi as f64);
            }
        }
    }
    ...
    fn notch_not_suggested_when_post_filter_is_clean() {
        ...
        assert!(
            r.iter()
                .all(|x| !x.param.name().starts_with("INS_HNTCH_FREQ")),
            "{:?}",
            r.iter().map(|x| x.param.name()).collect::<Vec<_>>()
        );
    }
    ...
    fn quicktune_oscillation_backs_off_only_that_axis() {
        let b = bundle(
            vec![],
            vec![
                step(Axis::Roll, 1.05, 20.0, 1.0),
                step(Axis::Pitch, 1.05, 20.0, 1.0),
            ],
        );
        let r = pids(&tune(), &b, Some([5.0, 1.0, 1.0]));
        let rp = r
            .iter()
            .find(|x| x.param.name() == "ATC_RAT_RLL_P")
            .unwrap();
    ...
    fn overshoot_raises_d_and_clamps_to_pdef_range() {
        ...
        let d = r
            .iter()
            .find(|x| x.param.name() == "ATC_RAT_PIT_D")
            .unwrap();
    ...
    fn autotune_min_d_fingerprint() {
        ...
        assert!(r
            .iter()
            .any(|x| x.param.name() == "AUTOTUNE_AGGR" && x.reason.contains("AUTOTUNE_MIN_D")));
    }
    ...
    fn saturation_blocks_gain_increase() {
        ...
        assert!(r
            .iter()
            .all(|x| x.param.name() != "ATC_RAT_RLL_I" || x.new.as_f64() <= 0.135));
    }
}
