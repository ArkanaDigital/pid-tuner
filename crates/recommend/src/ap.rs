...
mod tests {
    ...
    fn bundle(peaks: Vec<NoisePeak>, steps: Vec<StepResponse>) -> AnalysisBundle {
        AnalysisBundle { log: LogId("x".into()), quality: LogQuality { max_pid_out: Some([0.3, 0.3, 0.2]), has_pid_terms: true, ..Default::default() }, steps, spectra: vec![], spectrograms: vec![], peaks, anomalies: vec![], freq_resp: vec![] }
    }
