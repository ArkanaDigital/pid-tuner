...
pub struct FirmwareVersion {
    /// u16 so calendar versions such as Betaflight `2025.12.0` fit.
    pub major: u16,
    pub minor: u8,
    pub patch: u8,
}
...
impl From<Firmware> for InternalFirmware {
    fn from(fw: Firmware) -> Self {
        ...
        match fw {
            ...
            // pid_tuner patch: 4.6+ and calendar versions (2025.12+) use the
            // 4.5 field semantics until proven otherwise.
            Firmware::Betaflight(v) if v >= FirmwareVersion::new(4, 6, 0) => Self::Betaflight4_5,
